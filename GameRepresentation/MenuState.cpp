...
void MenuState::init() {
    if(!font.loadFromFile("../fonts/arial.ttf") or !background.loadFromFile("../pics/image.gif")){
    ...
    menuScreen[2].setString("HIGHSCORES");
    menuScreen[2].setFont(font);
    menuScreen[2].setFillColor(sf::Color::Red);
    menuScreen[2].setPosition(width / 2 -100, height / 3);


    ifstream file("savedHighscores.txt");
    stringstream s;
    s << file.rdbuf();

    menuScreen[3].setString(s.str());
    menuScreen[3].setFont(font);
    menuScreen[3].setFillColor(sf::Color::Red);
    menuScreen[3].setPosition(width / 2 -100, height / 2.5);
    ...
    while(renderWindow.pollEvent(event)){
        ...
        if(event.type == sf::Event::KeyPressed){
            ...
            if(event.key.code == sf::Keyboard::Key::Return){
                if(menuSelect == 0){
                    stateManager.addState(make_shared<LevelState>(renderWindow, stateManager, width, height));
                }
